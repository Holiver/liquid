...
class AbstractBlock extends AbstractTag
{
	...
	public function parse(array &$tokens) {
		...
		$tagRegexp = new Regexp('/^' . Liquid::get('TAG_START') . '\s*(\w+)\s*(.*)?' . Liquid::get('TAG_END') . '$/s');
	 ...
	 * @return string
	 */
	protected function renderAll($list, Context $context) {
		$result = '';
		if (!$list || empty($list)) {
		    return $result;
        }
		...
		foreach ($list as $token) {
			// $result .= (is_object($token) && method_exists($token, 'render')) ? var_export($token->render($context), true) : $token;
            ...
            if (is_object($token) && method_exists($token, 'render')) {
                ...
                if ($ret === true) {
                    ...
                    // 特殊情况硬编码
                    $result .= $this->outArray($ret);
                } elseif (is_object($ret) && method_exists($ret, '__toString')) {
                    $result .= $ret->__toString();
                } else {
                    $result .= $ret;
                }
            } else {
...
//        var_dump($result);
	...
	public function outArray($arr) {
	    ...
	    if (array_key_exists('one', $arr)) {
	        $r = preg_match('/^\s*(\d+)/', $arr['one'], $match);
	        if ($r == 1 && $match[1] == '1') {
	            return $arr['one'];
            }
            return $arr['other'];
