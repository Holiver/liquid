...
class Context
{
	...
	/**
	 * data shared between different render calls
	 *
	 * @var array
	 */
	public $shareData = array();

    const dianjiangImg = 'dianjiang://shop_image/';
	 ...
	 */
	public function __construct(array $assigns = array(), array $registers = array(), array &$shareData = array()) {
    ...
    private function variable($key) {
        if (!preg_match_all("/([a-zA-Z0-9\s_-]+|\[[a-zA-Z0-9\s_\-\.]+\])/", $key, $matches)) {
            return null;
        }
        ...
        foreach ($matches[1] as $match) {
            if (preg_match("/\[([a-zA-Z0-9\s_\-\.]+)\]/i", $match, $m)) {
                $index = trim($m[1]);
                array_push($parts, is_numeric($index) ? $index : $this->resolve($index));
            } else {
        ...
        while (count($parts) > 0) {
            ...
            if (is_array($object)) {
                ...
                $object = $this->specialDrop($object);
                if ($object instanceof Drop) {
                    if (!$object->hasKey($nextPartName)) {
                        return null;
                    }
...
//        }
     ...
     * @return Image
     */
    public function specialDrop($val) {
        if (is_string($val) && strpos($val, self::dianjiangImg) === 0) {
            $path = substr($val, strlen(self::dianjiangImg));
            $val = new Image($path);
        }
        return $val;
    }
