...
class Decision extends AbstractBlock
{
	...
	protected function interpretCondition($left, $right, $op = null, Context $context) {
        ...
        // values of 'blank'
        // '', "", false, empty object => blank
        if ($left == 'blank' || $right == 'blank') {
		    if ($left == 'blank') {
                $content = $context->get($right);
            } else {
                $content = $context->get($left);
            }
		...
		switch ($op) {
			...
			case 'contains':
				return is_array($left) ? in_array($right, $left) : (strlen($right) > 0 ? (strpos($left, $right) !== false) : false);
